	...
	$.fn.countdown.begin = function (parent, settings) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				container = $('<div/>').addClass('col-3 col-sm-3 col-md-3 col-lg-3').attr('id', k);
			...
			$.each(timespan, function (k, v) {
				...
				if(k == "days"){$('.label', '#' + k).text("Dias");}
				else if(k == "hours"){$('.label', '#' + k).text("Horas");}
				else if(k == "minutes"){$('.label', '#' + k).text("Minutos");}
				else if(k == "seconds"){$('.label', '#' + k).text("Segundos");}
				else if(k == "second"){$('.label', '#' + k).text("Segundo");}
			});
		}
